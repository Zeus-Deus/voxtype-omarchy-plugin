...
- Honest states: Voxtype not installed, voxtype-tui missing, daemon stopped, stale config, model missing, empty lists, bridge errors in the footer.
...
An installed Omarchy Quattro shell (`qs.Ui` / `qs.Commons`), `voxtype`, `/usr/bin/python3` with the `voxtype-tui` package (AUR: `voxtype-tui`), `zenity` for the import file chooser, and `omarchy-launch-terminal` for the GPU sudo hand-off. Git is needed for plugin-manager installation. Node.js and pytest are only needed for the development tests.
